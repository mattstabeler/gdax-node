...
var tickEmitter = new EventEmitter();
...
var OrderbookSync = function(productID, apiURI, websocketURI, authenticatedClient) {
  ...
  console.log('OrderbookSync:' + self.productID);
  ...
  WebsocketClient.call(self, self.productID, self.websocketURI, authenticatedClient);
  self.loadOrderbook();
};
...
_.assign(OrderbookSync.prototype, new function() {
  ...
  prototype.processMessage = function(data) {
    ...
    switch (data.type) {
      ...
      case 'match':
        self.tick(data.time, data.price, data.size, data.order_id);
  ...
  prototype.tick = function(time, price, size){
    tickEmitter.emit('tick', {time: time, price: price, size: size});
  };
